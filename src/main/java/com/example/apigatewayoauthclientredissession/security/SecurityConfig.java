...
//				.permitAll().pathMatchers("/index.html","/ui/**")
...
//				.permitAll()
//				//.pathMatchers("/actuator/gateway/routes").hasAnyRole("developer","admin")
//				.pathMatchers("/api/**").authenticated()).oauth2ResourceServer().jwt();
//		return http.build();
//		// @formatter:on
//	}
//	
	
	//IMP: We have defiend two beans for SecurityWebFilterChain because this API gateway is acting as both OAuth client and resource server. 
	// So, all the path starts with "/ui" will be protected using oauth authorization_grant flow. Path starts with "/api" will be protected using JWT token in authorization header.
	@Bean
	SecurityWebFilterChain configureSecurityFilterChainUI(ServerHttpSecurity http) throws Exception {
		http
        .authorizeExchange().pathMatchers("/users/status/check/unprotected","/actuator/health","/whoami","/index-cookie-localhost","/api/**").permitAll().pathMatchers("/index.html","/ui/**")
        .authenticated()
        .and()
        .oauth2Login();
		return http.build();
	...
	@Bean
	SecurityWebFilterChain configureSecurityFilterChainAPI(ServerHttpSecurity http) throws Exception {
		 http
         .authorizeExchange().pathMatchers("/users/status/check/unprotected","/actuator/health","/whoami","/index-cookie-localhost","/ui/**").permitAll().pathMatchers("/api/**")
         .authenticated()
         .and()
